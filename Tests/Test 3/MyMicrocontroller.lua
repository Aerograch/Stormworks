...
function onTick()
    ...
    controller.gamepad = input.getNumber(constants.gamepadSwitchButton)
    ...
    if controller.gamepad then
        ...
        controller.back = clamp(input.getNumber(constants.keyboardThrottleAxi)*-1, 0, 1)
    ...
    if car.wheelRPS < 0.1 and controller.front > 0.05 and car.reverseSwitchDelay == 0 then
        car.reverse = false
    ...
    if car.reverse then
        ...
        if constants.smoothBreaking then
            car.brakes = clamp((controller.front-0.05)/0.95, 0, 1) > 0 and math.min(1, car.brakes + 0.02) or math.max(0, car.brakes - 0.02)
        else
            car.brakes = clamp((controller.front-0.05)/0.95, 0, 1)
        end
    else
        if constants.smoothBreaking then
            car.brakes = clamp((controller.back-0.05)/0.95, 0, 1) > 0 and math.min(1, car.brakes + 0.02) or math.max(0, car.brakes - 0.02)
        else
            car.brakes = clamp((controller.back-0.05)/0.95, 0, 1)
        end
        car.throttle = clamp((controller.front-0.05)/0.95, 0, 1)
    end
    ...
    if car.parkingBreak or (car.wheelRPS < 1 and car.throttle == 0) then
        car.clutch = math.max(0, car.clutch-(1/constants.clutchTurnoverTimeframe))
        car.throttle = constants.isModular and 0 or 5-car.engineRPS
    else
